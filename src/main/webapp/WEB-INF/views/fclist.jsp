...
<p>
<input id="filter" type="button" value="옵션닫기" onclick="openfilter()">
</p>
...
<table border=1 height="50px" width="500px" align="center">
	<tr>
		...
		<td>
 			<table>
				<tr>
					<td width=33%><input type="checkbox" name="size" value="1" <c:forEach items="${scat}" var="ssize">
						${ssize eq '1' ? 'checked' : '' }</c:forEach>>소형 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="checkbox" name="size" value="2" <c:forEach items="${scat}" var="ssize">
						${ssize eq '2' ? 'checked' : '' }</c:forEach>>중형 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="checkbox" name="size" value="3" <c:forEach items="${scat}" var="ssize">
						${ssize eq '3' ? 'checked' : '' }</c:forEach>>대형</td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr>
		...
		<table>
			...
			<c:forEach items="${comp }" var="company">
				...
				%>
				<input type="checkbox" name="comp" value="<%=co%>" <c:forEach items="${scom}" var="scompany">
						${company eq scompany ? 'checked' : '' }</c:forEach>><%=co%>
				<%if (count % 3 == 0) {%>
					</tr><tr>
				<%} %>
			</c:forEach>
...
<p align=center><input type="submit" value="필터적용">&nbsp;&nbsp;&nbsp;
	<input type="button" onclick="location.href='carList'" value="필터취소"></p>
</form>
</div>
<h2 class = 'col' align=center>차량정보</h2>
<table border=1 align="center"width=100%>
	...
	<c:forEach items="${fclist }" var="dto">	<tr class="link" onclick="location.href='carinfo?no=${dto.no}'">
		<%
